		...
		namespace I2C
		{
			class Master
			{
			  public:
				...
				Master(uint8_t interface) : interface(interface) { twi_init(); };
				...
				void address(uint8_t slave) { this->slave = slave; };

				/* Read data into array specified. Returns number of bytes read */
				uint8_t read(uint8_t* data, uint8_t lenght, bool stopsign) { return twi_readFrom(slave, data, lenght, stopsign); };
				...
				void speed(uint32_t frequency) { twi_setFrequency(frequency); };
